...
describe('board engine',()=>{
  ...
  it('draws and resets cards from a deck',()=>{const deck:DeckObject={...counter('deck'),type:'deck',cards:['A0','K1'],initialCards:['A0','K1']};const drawn=applyCommand(state([deck]),{type:'drawCard',id:'deck'},'sam');expect((drawn.objects[0] as DeckObject).cards).toEqual(['A0']);expect(drawn.objects[1]).toMatchObject({type:'card',rank:'K',suit:'♥'});const reset=applyCommand(drawn,{type:'resetDeck',id:'deck'},'sam');expect(reset.objects).toHaveLength(1);expect((reset.objects[0] as DeckObject).cards).toHaveLength(2);});
});
